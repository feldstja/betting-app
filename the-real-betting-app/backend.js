  ...
  socket.on('getBets', (data) => {
    Bets.find((err, bets)=>{
      if(err){
        ...
        var theBets= [{Text: 'saquon barkley will rush for 115 yards', Amount: 20, Odds: 1, Opponent: "none", Creator: 'Sup', CreatorWon: 0}]
  ...
  socket.on('getProfBets', (user) => {
    Bets.find({Creator: user.Username}, (err, bets)=>{
      if(err){
        ...
        var theBets= [{Text: 'saquon barkley will rush for 115 yards', Amount: 20, Odds: 1,  Creator: 'Sup', Opponent: "none", CreatorWon: 0}]
        socket.emit('loadProfBets', theBets)
      } else {
        bets = bets.filter(function (a){
          if(a.CreatorWon === 0){
            return 1;
          } else{
            return -1;
    ...
    socket.on('wonBet', (data)=>{
      ...
      .then(theBet=>{
        ...
        .then(()=>{
          ...
          .then(theUser => {
            socket.emit('updatedUser', theUser)
          })
        })
        .then(()=>{
          if(data.bet.Opponent !== ''){
